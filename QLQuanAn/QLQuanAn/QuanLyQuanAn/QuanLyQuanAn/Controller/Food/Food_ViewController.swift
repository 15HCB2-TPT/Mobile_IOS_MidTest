...
class Food_ViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UIPickerViewDelegate {
    ...
    @IBAction func btnTim_TouchUpInside(_ sender: Any) {
        ...
        if(txtTen.text != "") { conditions.append(conditionName) }
        if(swtTimNC.isOn) {
            if(txtLoaiMonAn.text != "") { conditions.append(conditionType) }
            if(txtTu.text != "" && txtDen.text != "") { conditions.append(conditionMoney) }
        }
